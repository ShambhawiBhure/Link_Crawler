...
#CONSTANTS
MAX_DATA_LIMIT=5000
CRAWL_AFTER=datetime(2021,5,23)-datetime(2021,5,22)
DELAY_TIME=5
ROOT_URL= config['ROOT_URL']

#Connecting to MongoDB
cluster=MongoClient(port = config['PORT'])
db=cluster[config['COLLECTION_NAME']]
...
def crawl_data(data_list,DELAY_TIME,CRAWL_AFTER,MAX_DATA_LIMIT,collection,new=True):
    ...
    for data in data_list:
        ...
        try:
            ...
            # if response is not html then call other content type
            else:
...
while 1:
    ...
    # crawl for data that do not crawled for last one day
